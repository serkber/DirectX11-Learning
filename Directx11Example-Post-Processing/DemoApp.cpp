...
DemoApp::DemoApp()
{
    ...
    m_pBackgroundMapResource = nullptr;
}
...
bool DemoApp::LoadVertexShader(const wchar_t* file, ID3D11VertexShader** shader, ID3DBlob** pVSBuffer)
{
    LPCWSTR shaderError = L"";
    HRESULT hr;

    //Load vertex Shader
    if (!CompileShader(file, "vsmain", "vs_5_0", pVSBuffer, &shaderError))
    {
...
bool DemoApp::LoadPixelShader(const wchar_t* file, ID3D11PixelShader** shader)
{
    LPCWSTR shaderError = L"";
    HRESULT hr;
    ...
    if (!CompileShader(file, "psmain", "ps_5_0", &pPSBuffer, &shaderError))
    {
...
bool DemoApp::LoadTextures()
{
    ...
    hr = DirectX::CreateDDSTextureFromFile(m_pD3DDevice, m_pD3DContext, Textures[Crosshair], &m_pColorMapResource, &m_pColorMap);
    ...
    hr = DirectX::CreateDDSTextureFromFile(m_pD3DDevice, m_pD3DContext, Textures[Dragon], &m_pBackgroundMapResource, &m_pBackgroundMap);
...
bool DemoApp::CreateConstantBuffer()
{
    ...
    constDesc.ByteWidth = sizeof(CBuffer);
...
bool DemoApp::LoadContent()
{
    ...
    if (!LoadVertexShader(L"VertexShader.hlsl", &m_pVertexShader, &pVSBuffer))
    {
        return false;
    }
    
    if (!LoadVertexShader(L"PostProcessVShader.hlsl", &m_pPostProcessVShader, &pVSBuffer))
    {
        return false;
    }
    ...
    if (!LoadPixelShader(L"PixelShader.hlsl", &m_pPixelShader))
    {
        return false;
    }
    if (!LoadPixelShader(L"PostProcessShader.hlsl", &m_pPostProcessShader))
    {
        return false;
    }
...
void DemoApp::UnloadContent()
{
    ...
    m_pColorMap = NULL;
    if (m_pBackgroundMap)
        m_pBackgroundMap->Release();
    m_pBackgroundMap = NULL;
    if (m_pBackgroundMapResource)
        m_pBackgroundMapResource->Release();
    m_pBackgroundMapResource = nullptr;
}
...
void DemoApp::Render()
{
    ...
    m_pD3DContext->VSSetShader(m_pPostProcessVShader, 0, 0);
    m_pD3DContext->PSSetShaderResources(0, 1, &m_pBackgroundMap);
    ...
    CBuffer cBuffer {crosshairMvp, m_elapsedTime, m_mousePosNorm};    
    m_pD3DContext->UpdateSubresource( m_pConstantBuffer, 0, 0, &cBuffer, 0, 0 );

    //PostProcess
    m_pD3DContext->CopyResource(m_pPostProcessTexture, m_pBackBuffer);
    m_pD3DContext->VSSetShader(m_pPostProcessVShader, 0, 0);
    m_pD3DContext->PSSetShader(m_pPostProcessShader, 0, 0);
    m_pD3DContext->PSSetShaderResources(0, 1, &m_pPostProcessResource);
    m_pD3DContext->Draw(6, 0);
